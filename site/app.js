...
function shareUrl(slug) {
  return `${SITE_BASE_URL}/s/${encodeURIComponent(slug)}.html`;
}

// A small button that copies a skill's static, real-metadata share URL
// (site/s/<slug>.html, generated by scripts/build-index.js) rather than the
// skill.html?slug= URL used for in-site browsing — the former unfurls
// correctly on Slack/Twitter/etc., the latter is JS-rendered and won't.
...
// Builder) that don't otherwise sit in the header nav. One overlay is built
// lazily on first use and reused for the rest of the page's life;
// bindCommandPalette() just needs to be called once per page, after that
// page's data has loaded, with the full skills list in scope.
const PALETTE_PAGES = [
  { title: "Workflow Builder", url: "workflow.html", tag: "page" },
  { title: "What's New — version history", url: "whats-new.html", tag: "page" },
];

let paletteState = null;

function buildCommandPalette(skills) {
  ...
  const items = [
    ...
    ...PALETTE_PAGES.map((p) => ({ ...p, searchText: p.title })),
  ];
  ...
  function render(query) {
    ...
    state.matches = !q ? items : items.filter((it) => it.searchText.toLowerCase().includes(q));
    state.activeIndex = 0;
    list.innerHTML = state.matches
      .map(
        (it, i) =>
          `<li class="command-palette-result${i === 0 ? " active" : ""}" data-url="${escapeHtml(it.url)}">
...
function bindCommandPalette(skills) {
  buildCommandPalette(skills);
